                        ...
                        <div class="card-body">
                            <div class="p-4 border rounded">
                                <form class="row g-3" method="POST" action="{{ $route }}">
                                    ...
                                    <div class="col-md-12">
                                        <label for="name"
                                               class="form-label">{{__('Navn på fast program')}}*</label>
                                        <input type="text" name="name" class="form-control" id="name"
                                               value="{{ old('name',$setupFixedProgram->name) }}" minlength="3"
                                               placeholder="{{__('custom.enter')}} {{__('navn på fast program')}}" required>
                                        @error('name')
...
{{--                                                <option--}}
{{--                                                    {{$setupFixedProgram->group_id == $item->id ? 'selected' : ''}} value="{{$item->id}}">{{ ucfirst($item->name)}}</option>--}}
{{--                                            @empty--}}
...
{{--                                                <option--}}
{{--                                                    {{$setupFixedProgram->exercise_category_id == $item->id ? 'selected' : ''}} value="{{$item->id}}">{{ ucfirst($item->name)}}</option>--}}
{{--                                            @empty--}}
{{--                                                <option disabled value="">Not Found!</option>--}}
{{--                                            @endforelse--}}
{{--                                        </select>--}}
...
{{--                                               value="{{ old('sort',$setupFixedProgram->sort) }}" min="1"--}}
...
{{--                                    </div>--}}
                                    ...
                                    <div class="col-md-12 d-none">
                                        ...
                                        <input type="text" name="comment" class="form-control" id="comment"
                                               value="{{ old('comment',$setupFixedProgram->comment) }}"
