		...
		<div class="page-wrapper">
			...
			<div class="page-content">
				...
				<div class="page-breadcrumb d-none d-sm-flex align-items-center mb-3">
					<div class="breadcrumb-title pe-3">{{__('custom.intensity')}}</div>
					<div class="ps-3">
						<nav aria-label="breadcrumb">
							<ol class="breadcrumb mb-0 p-0">
								<li class="breadcrumb-item"><a href="{{route('fullcalender.id','all')}}"><i class="bx bx-home-alt"></i></a>
								</li>
								<li class="breadcrumb-item active" aria-current="page">{{__('custom.intensity')}} {{__('custom.list')}}</li>
							</ol>
						</nav>
					</div>
					<div class="ms-auto">
						<div class="btn-group">
							<a href="{{route('intensity.create')}}" class="btn btn-primary">{{__('custom.add')}}</a>
						</div>
					</div>
				</div>
				<!--end breadcrumb-->
...
{{--                                        <td>{{date('d-M-Y' , strtotime($item->created_at))}}</td>--}}
                                        ...
                                        <td>
                                            <a href="{{route('intensity.edit',['intensity' => $item->id ])}}" class="btn btn-info text-white">{{__('custom.edit')}}</a>
