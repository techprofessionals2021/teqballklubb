            ...
            <div class="row">
                <div class="col-lg-12 text-center pb-2">
                    <div class="page-title">
                        ...
                        @elseif(count($userSetupFixedPrograms) != 0)
                            <h3>Mine øvelser for {{ucwords($userSetupFixedPrograms->first()->name)}}</h3>
                        @else
                    ...
                    <div class="dropdown">
                        <button class="btn btn-secondary dropdown-toggle" type="button" id="dropdownMenuButton" data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
                            ...
                            @elseif(count($userSetupFixedPrograms) != 0)
                                {{ucwords($userSetupFixedPrograms->first()->name)}}
                            @endif
                        </button>
        ...
        <div class="col-lg-12">
            <div class="row">
                ...
                @if(count($userPrograms) != 0)
                    ...
                    @endif
                @elseif(count($userSetupFixedPrograms) != 0)
                    @if(!is_null($userSetupFixedPrograms->first()))
                        @if(count($userSetupFixedPrograms->first()->setupProgramsByRegProgram) != 0)
                            @foreach($userSetupFixedPrograms->first()->setupProgramsByRegProgram->sortBy('sort') as $item)
                                {{--                            @continue($item->user_id != auth()->id())--}}
                        ...
                        <div class="modal-content">
                            ...
                            <div class="modal-body">
                                ...
                                <form class="row g-3" id="registerProgform">
                                    ...
                                    <div class="col-md-4">
                                        <input type="number" name="hastighet" class="form-control" id="hastighet"
                                               value="" min="0" placeholder="Hastighet(Km/h)">
                                    </div>
                                </form>
                            </div>
                            <div class="modal-footer">
        ...
        function submitRegisterExerciseProgressRecord() {
            ...
            $.ajax({
                ...
                data: {
                    ...
                    break: dbreak, distance: distance, puls: puls,bevegelighet: bevegelighet,
                    hastighet:hastighet,
                     _token: "{{csrf_token()}}"},
                success: function (response) {
                    if (response.status) {
